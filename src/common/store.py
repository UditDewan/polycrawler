...
_VERBS = {"replace": "INSERT OR REPLACE", "ignore": "INSERT OR IGNORE"}


def _insert(con, table: str, cols: list[str], rows: list[dict], *, mode: str) -> int:
    if not rows:
        return 0
    data = []
    for r in rows:
...
# Idempotent inserts for re-runnable ingestion (rely on the PRIMARY KEYs):
